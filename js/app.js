...
function set_api_data(name, mass, volume, gravity){
	if (information.status=="successful") {
		...
		masa_text.setAttribute('value',"Masse: "+information.mass+" kg")
		volumen_text.setAttribute('value',"Volume: "+information.volume+" km3")
		gravedad_text.setAttribute('value',"Gravite: "+information.gravity+" m/s2")
	}else{
		planeta_text.setAttribute('value',name)
		masa_text.setAttribute('value',"Masse: "+mass+" kg")
		volumen_text.setAttribute('value',"Volume: "+volume+" km3")
		gravedad_text.setAttribute('value',"Gravite: "+gravity+" m/s2")
	}
}
...
AFRAME.registerComponent('do-something', {
  init: function () {
    ...
    mercurio.addEventListener('click', function (evt) {
    	var name = "Mercury"
    	get_api_data(name)
    	setTimeout( () => {set_api_data("Mercure","3,302*10 23","6,083*10 10","3,7")}, 1000)
    ...
    tierra.addEventListener('click', function (evt) {
    	var name = "Earth"
    	get_api_data(name)
    	setTimeout( () => {set_api_data("Terre","5,9736*10 24","1,08321*10 12","9,780327")}, 1000)
	    datos.object3D.visible = true
	    setTimeout( () => {datos.object3D.visible = false}, 8000)
	});
	...
	saturno.addEventListener('click', function (evt) {
		var name = "Saturn"
		get_api_data(name)
    	setTimeout( () => {set_api_data("Saturne","5,688x10 26","8,27x10 23","10,44")}, 1000)
	    datos.object3D.visible = true
	    setTimeout( () => {datos.object3D.visible = false}, 8000)
	});
